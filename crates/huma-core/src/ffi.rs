...
impl FFIManager {
    ...
    pub fn yukle(&mut self, ad: &str, yol: &str) -> Result<(), String> {
        unsafe {
            match Library::new(yol) {
                ...
                Err(e) => Err(format!("C/C++/CUDA kütüphanesi yüklenemedi ('{}'): {}", yol, e)),
            }
        }
    }

    pub fn cagir_f64_f64(&self, lib_ad: &str, fn_ad: &str, arg: f64) -> Result<f64, String> {
        let lib = self.libraries.get(lib_ad)
    ...
    pub fn cagir_f64_f64_f64(&self, lib_ad: &str, fn_ad: &str, arg1: f64, arg2: f64) -> Result<f64, String> {
        let lib = self.libraries.get(lib_ad)
            .ok_or_else(|| format!("Yüklenmemiş FFI kütüphanesi: {}", lib_ad))?;
        unsafe {
    ...
    pub fn cagir_esnek(&self, lib_ad: &str, fn_ad: &str, argumanlar: Vec<Deger>) -> Result<Deger, String> {
        let lib = self.libraries.get(lib_ad)
            .ok_or_else(|| format!("Yüklenmemiş FFI kütüphanesi: {}", lib_ad))?;
        ...
        unsafe {
            match argumanlar.len() {
                ...
                1 => {
                    match &argumanlar[0] {
                        ...
                        Deger::Metin(s) => {
                            ...
                            let func: Symbol<unsafe extern "C" fn(*const std::os::raw::c_char) -> *const std::os::raw::c_char> = lib
                                .get(fn_ad.as_bytes())
                                .map_err(|e| format!("FFI Sembolü bulunamadı ('{}'): {}", fn_ad, e))?;
                ...
                _ => Err(format!("FFI çağrısında {} argüman henüz desteklenmiyor", argumanlar.len())),
            }
        }
    }
}
