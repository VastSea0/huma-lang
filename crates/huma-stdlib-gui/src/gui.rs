...
use huma_runtime::value::{BuiltinRuntime, Deger};
...
pub fn kayit_et(globals: &mut HashMap<String, Deger>) {
    if capability::require(Capability::Gui, "GUI yerleşikleri").is_err() {
        return;
    }
    ...
    globals.insert(
        ...
        Deger::DahiliFonksiyon(|args| {
            ...
            let list = TEMA_ADLARI
                ...
                .collect();
            Deger::Liste(Gc::from_cell(RefCell::new(list)))
        }),
    );
    ...
    globals.insert(
        ...
        Deger::DahiliFonksiyon(|args| {
            let [Deger::Sayi(active), Deger::Metin(text)] = args.as_slice() else {
                return Deger::Hata(
                    "radyo_düğmesi: 0/1 seçili durumu ve metin gerekir".to_string(),
                );
            };
        ...
        Deger::DahiliFonksiyon(|args| {
            ...
            for item in options_ref.iter() {
                match item {
                    ...
                    _ => {
                        return Deger::Hata(
                            "açılır_liste: liste yalnızca metin içermelidir".to_string(),
                        )
                    }
                }
            }
            ...
            if !selected.is_finite() || *selected < 0.0 || selected.fract() != 0.0 {
                return Deger::Hata(
                    "açılır_liste: seçili indeks negatif olmayan bir tamsayı olmalıdır".to_string(),
                );
            }
        ...
        Deger::DahiliFonksiyon(|args| {
            ...
            match with_ui("renk_seçici", |ui| {
                ...
                Ok(()) => {
                    ...
                    Deger::Liste(Gc::from_cell(RefCell::new(values)))
                }
                Err(error) => Deger::Hata(error),
            }
        }),
