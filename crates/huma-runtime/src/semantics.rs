...
pub fn esit_mi(left: &Deger, right: &Deger) -> Result<bool, String> {
    fn compare(
        ...
        match (left, right) {
            ...
            (Deger::Liste(a), Deger::Liste(b)) => {
                ...
                let pair = (Gc::as_ptr(a) as usize, Gc::as_ptr(b) as usize, 1);
            ...
            (Deger::Harici(a), Deger::Harici(b)) => Ok(a == b),
            (
                Deger::Fonksiyon { .. }
                | Deger::BytecodeFonksiyon { .. }
                | Deger::DahiliFonksiyon(_)
    ...
    fn compare_maps(
        left: &Gc<std::collections::HashMap<String, Deger>>,
        right: &Gc<std::collections::HashMap<String, Deger>>,
        active: &mut HashSet<(usize, usize, u8)>,
        depth: usize,
        kind: u8,
    ) -> Result<bool, String> {
        ...
        let pair = (Gc::as_ptr(left) as usize, Gc::as_ptr(right) as usize, kind);
        if !active.insert(pair) {
            return Ok(true);
        }
        let left = left
...
mod tests {
    ...
    fn bos_sozluk_ve_liste_yanlistir() {
        assert!(!dogru_mu(&Deger::Liste(Gc::new(Vec::new()))).unwrap());
        assert!(!dogru_mu(&Deger::Sozluk(Gc::new(HashMap::new()))).unwrap());
    }
    ...
    fn dongusel_listeler_esitlikte_yigin_tasirmadan_karsilastirilir() {
        let left = Gc::new(Vec::new());
        let right = Gc::new(Vec::new());
        left.borrow_mut().push(Deger::Liste(Gc::clone(&left)));
        right.borrow_mut().push(Deger::Liste(Gc::clone(&right)));
        assert!(esit_mi(
            &Deger::Liste(Gc::clone(&left)),
            &Deger::Liste(Gc::clone(&right))
        )
    ...
    fn aktif_borclu_koleksiyon_sessiz_sonuca_donusmez() {
        let list = Gc::new(vec![Deger::Sayi(1.0)]);
        let other = Gc::new(vec![Deger::Sayi(1.0)]);
        let _borrow = list.borrow_mut();
        assert!(dogru_mu(&Deger::Liste(Gc::clone(&list)))
            .expect_err("Doğruluk borrow çakışmasını bildirmeli")
            .contains("kullanımda"));
        assert!(
            esit_mi(&Deger::Liste(Gc::clone(&list)), &Deger::Liste(other))
