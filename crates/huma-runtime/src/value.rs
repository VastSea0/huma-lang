...
/// Alan adaptörlerinin çekirdeğe yeni enum varyantı eklemeden taşıyabildiği
/// tür-silinmiş değer sözleşmesi.
pub trait HostValue: std::fmt::Debug + Send + Sync + 'static {
    fn as_any(&self) -> &dyn Any;
    fn type_label(&self) -> String;
    fn display(&self) -> String;
    fn equals(&self, other: &dyn HostValue) -> bool;
}

#[derive(Clone)]
pub struct HostObject(Arc<dyn HostValue>);
...
impl HostObject {
    ...
    pub fn downcast_ref<T: HostValue>(&self) -> Option<&T> {
...
pub enum Deger {
    ...
    Nesne {
        sinif_adi: String,
        alanlar: Gc<HashMap<String, Deger>>,
        module_kimligi: Option<String>,
    },
    Sozluk(Gc<HashMap<String, Deger>>),
    Hata(String),
...
fn deger_yaz(
    ...
    match value {
        ...
        Deger::Liste(l) => {
            let identity = Gc::as_ptr(l) as usize;
        ...
        Deger::Sozluk(m) => {
            let identity = Gc::as_ptr(m) as usize;
            if !active.insert(identity) {
        ...
        Deger::Harici(value) => write!(f, "{}", value.display()),
    ...
    pub fn to_json_checked(&self) -> Result<serde_json::Value, String> {
        ...
        fn convert(
            ...
            match value {
                ...
                Deger::Liste(items) => {
                    let identity = Gc::as_ptr(items) as usize;
                    if !active.insert(identity) {
                ...
                Deger::Harici(value) => Err(format!(
                    "Haricî '{}' değeri JSON ile temsil edilemez",
                    value.type_label()
                )),
            }
        }
